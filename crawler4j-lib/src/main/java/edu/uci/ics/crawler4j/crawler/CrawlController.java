...
public class CrawlController extends Configurable {
    ...
    public <T extends WebCrawler> void start(final Class<T> _c, final int numberOfCrawlers) {
        this.start(new DefaultWebCrawlerFactory<T>(_c), numberOfCrawlers, true);
    }

    /**
     * Start the crawling session and wait for it to finish.
     *
     * @param factory          the factory instance which will be used to create WebCrawler objects
     * @param numberOfCrawlers the number of concurrent threads that will be contributing in
     *                         this crawling session.
     */
    public <T extends WebCrawler> void start(final WebCrawlerFactory<T> factory, final int numberOfCrawlers) {
        this.start(factory, numberOfCrawlers, true);
    }
    ...
    public <T extends WebCrawler> void startNonBlocking(final Class<T> _c, final int numberOfCrawlers) {
        this.start(new DefaultWebCrawlerFactory<T>(_c), numberOfCrawlers, false);
    }
     ...
     *                         this crawling session.
     */
    public <T extends WebCrawler> void startNonBlocking(final WebCrawlerFactory<T> factory, final int numberOfCrawlers) {
        this.start(factory, numberOfCrawlers, false);
    }

    protected <T extends WebCrawler> void start(final WebCrawlerFactory<T> factory, final int numberOfCrawlers, boolean isBlocking) {
        try {
            finished = false;
